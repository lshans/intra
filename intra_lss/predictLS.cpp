...
long long predict_3_parameter(short block_image[8][8], short block_resi[4][4], short block_pre[4][4], int i_row, int i_col)
{
	...
	for(int mode = 0; mode < 9; ++mode)
	{
		estimate(block_image, para, mode);
...
long long predict(short **image_construct, short **resi, short **predicted, int height, int width)
{
	...
	for (int i_row = 0; i_row < ROWS; ++i_row)
	{
		for (int i_col = 0; i_col < COLS; ++i_col)
		{
			...
			//energy_4_parameter = predict_4_parameter(Para4_image[i_row][i_col], block_resi, predicted);
			//energy_1_parameter = predict_1_parameter(Para1_image[i_row][i_col], block_resi, predicted);
			//energy_2_parameter = predict_2_parameter(Para2_image[i_row][i_col], block_resi, predicted);
			...
			printf("Accumulated energe [%4d][%4d]  %I64d\n", i_row, i_col, energy_3_parameter);
			resi_energy += energy_3_parameter;
